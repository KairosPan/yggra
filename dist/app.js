(() => {
  ...
  function renderTree(){
    $$('[data-node]').forEach(el=>{const id=el.dataset.node,active=id===state.branch;el.classList.toggle('selected',active);if(active)el.setAttribute('aria-current','true');else el.removeAttribute('aria-current');el.setAttribute('aria-label',`${branches[id].title}: ${branches[id].papers.length} papers. Show branch details.`);el.href=href({view:'tree',branch:id,paper:branches[id].papers[0],query:''});el.classList.toggle('dim',state.question!=='all'&&!branches[id].papers.some(id=>relevant(getPaper(id))));});
  ...
  iconsIn();render();history.replaceState(null,'',href({}));
})();
